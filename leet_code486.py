...
class Solution(object):
    def PredictTheWinner(self, nums):
        ...
        self.dp = dict()
        def helper(nums, i, j):
            if self.dp.has_key((i, j)):
                return self.dp[(i, j)]

            if i == j:
                return nums[i]

            self.dp[(i, j)] = max(nums[i] - helper(nums, i+1, j), nums[j] - helper(nums, i, j-1))
            return self.dp[(i, j)]

        result = max(nums[0] - helper(nums, 1, len(nums)-1), nums[-1] - helper(nums, 0, len(nums)-2))
        return result >= 0
