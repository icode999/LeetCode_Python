...
You are playing a simplified Pacman game. You start at the point (0, 0), and your destination is (target[0], target[1]). There are several ghosts on the map,
...
Google
"""
# If distance to any ghost is less than distance to target, ghost wins, as it can wait at target before the player
class Solution(object):
    def escapeGhosts(self, ghosts, target):
        ...
        for g0, g1 in ghosts:
            if (abs(target[0] - g0) + abs(target[1] - g1)) < target_dist:
                return False
        return True